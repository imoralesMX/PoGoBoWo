                    ...
                    if pokemon['pokemon_id'] in CANDY_NEEDED_TO_EVOLVE:
                        for inventory_item in inventory_items:
                            if "pokemon_family" in inventory_item['inventory_item_data'] and (inventory_item['inventory_item_data']['pokemon_family']['family_id'] == pokemon['pokemon_id'] or inventory_item['inventory_item_data']['pokemon_family']['family_id'] == (pokemon['pokemon_id'] - 1)) and inventory_item['inventory_item_data']['pokemon_family'].get('candy', 0) > CANDY_NEEDED_TO_EVOLVE[pokemon['pokemon_id']] and pokemon['pokemon_id'] not in self.evolved_pokemon_ids:
                                self.log.info(colored("Evolving pokemon: %s", 'green'), self.pokemon_names[str(pokemon['pokemon_id'])])
                                ...
                                if pokemon_iv_percentage(pokemon) > pokemon_iv_percentage(last_pokemon):
                                    if last_pokemon['cp'] * self.DUPLICATE_CP_FORGIVENESS < pokemon['cp']:
                                        try:
                                            ...
                                            if atgym:
                                                self.log.info(colored("Pokemon %s CP: %s not released because at gym", 'cyan'), self.pokemon_names[str(last_pokemon['pokemon_id'])], last_pokemon['cp'])
                                        except:
                                            atgym = False
                                        if not atgym:
                                            self.log.debug("Releasing pokemon: %s", last_pokemon)
                                            self.log.info(colored("Releasing pokemon: %s IV: %s", 'cyan'), self.pokemon_names[str(last_pokemon['pokemon_id'])], pokemon_iv_percentage(last_pokemon))
                                    ...
                                    if last_pokemon['cp'] * self.DUPLICATE_CP_FORGIVENESS > pokemon['cp']:
                                        try:
                                            ...
                                            if atgym:
                                                self.log.info(colored("Pokemon %s not released because at gym", 'cyan'), self.pokemon_names[str(pokemon['pokemon_id'])])
                                        except:
                                            atgym = False
                                        if not atgym:
                                            self.log.debug("Releasing pokemon: %s", pokemon)
                                            self.log.info(colored("Releasing pokemon: %s IV: %s", 'cyan'), self.pokemon_names[str(pokemon['pokemon_id'])], pokemon_iv_percentage(pokemon))
